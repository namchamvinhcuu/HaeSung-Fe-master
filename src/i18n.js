...
export const AVAILABLE_LOCALES = window.__AVAILABLE_LOCALES__ || DEFAULT_LOCALES;

const defaultBundle = (locale, ns) => import(`./translations/${locale}/${ns}.json`).then((res) => res.default);

const dynamicBundle = (locale, ns) => fetch(`/locales/${locale}/${ns}.json`).then((res) => res.json());
