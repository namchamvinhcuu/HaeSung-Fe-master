...
function api_push_notify(user_id_list, message) {
  ...
  } else message.NoticeUserIdList = user_id_list;
  ...
  return new Promise((resolve, reject) => {
    fetch(ConfigConstants.BASE_URL + 'sysNotice/add', requestOptions)
      .then(handleResponse, handleError)
      .then((res) => {
        if (res.succeeded) {
          resolve(res.data);
...
function handleResponse(response) {
  ...
  else if (refresh_token && access_token && access_token !== 'invalid_token') {
    ...
    if (response.ok) {
      ...
      try {
        ...
        switch (response.status) {
            ...
            response.json().then((json) => {
              ...
              if (typeof json === 'object') {
                for (var propertyName in json) {
                  ...
                  } else errorMsg[propertyName] = property[0];
                }
          ...
          case 401:
            console.log(response);
            notifyError(`${TextConstants.TEXT_LOGIN_AGAIN_MSG}. Tự động chuyển hướng sang trang đăng nhập.`);
            emitRedirectSignIn({
              redirectUrl: '/login',
            });
          ...
          case 403:
            console.log(response);
            notifyError(`${TextConstants.TEXT_FORBIDDEN}. Tự động chuyển hướng sang trang đăng nhập.`);
            emitRedirectSignIn({
              redirectUrl: '/login',
            });
            return reject(response);
...
export { login, logout, api_get, api_post, api_download, api_post_formdata, api_push_notify };
