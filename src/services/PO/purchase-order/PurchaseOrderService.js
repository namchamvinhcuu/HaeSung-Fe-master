...
const downloadReport = async (params) => {
  try {
    ...
    fetch(
      `${ConfigConstants.API_URL}fixed-po/download-excel?PoCode=${params.PoCode}&DeliveryDate=${moment(
        params.DeliveryDate
      ).format('YYYY-MM-DD')}&DueDate=${moment(params.DueDate).format('YYYY-MM-DD')}&isActived=${params.showDelete}`,
      options
    ).then((response) => {
