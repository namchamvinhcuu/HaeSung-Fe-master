...
export const downloadWOReport = async (params) => {
  try {
    ...
    fetch(
      `${ConfigConstants.API_URL}mms-report/download-wo-report?WoCode=${params.WoCode}&StartSearchingDate=${moment(
        params.StartSearchingDate
      ).format('YYYY-MM-DD')}&EndSearchingDate=${moment(params.EndSearchingDate).format('YYYY-MM-DD')}&MaterialId=${
        params.MaterialId
      }`,
      options
    ).then((response) => {
...
export const downloadLotReport = async (params) => {
  try {
    ...
    fetch(
      `${ConfigConstants.API_URL}mms-report/download-lot-report?WoId=${params.WoId}&Status=${params.Status ?? ''}`,
      options
    ).then((response) => {
      response.blob().then((blob) => {
        let url = URL.createObjectURL(blob);
