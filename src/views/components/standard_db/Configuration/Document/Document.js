...
export default function Document() {
  ...
  const columns = [
    ...
    {
      field: 'urlFile',
      headerName: intl.formatMessage({ id: 'document.urlFile' }),
      flex: 0.8,
      renderCell: (params) => {
        return (
          <a
            href={`${BASE_URL}/document/${params.row.language}/${params.row.urlFile}`}
  ...
  const handleDelete = async (document) => {
    if (
      window.confirm(
        intl.formatMessage({ id: document.isActived ? 'general.confirm_delete' : 'general.confirm_redo_deleted' })
      )
    ) {
      try {
        let res = await documentService.deleteDocument({
  ...
  const handleDownload = async (row) => {
    ...
    fetch(`${API_URL}Document/download/${row.menuComponent}/${row.language}`, options).then((response) => {
