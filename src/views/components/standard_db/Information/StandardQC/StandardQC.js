...
const StandardQC = (props) => {
  ...
  useEffect(() => {
    if (!_.isEmpty(selectedRow) && !_.isEqual(selectedRow, StandardQCDto)) {
      ...
      const index = _.findIndex(newArr, function (o) {
        return o.QCId == selectedRow.QCId;
      });
      if (index !== -1) {
  ...
  const handleDelete = async (row) => {
    let message = standardQCState.searchData.showDelete
  ...
  const columns = [
    { field: 'QCId', headerName: '', flex: 0.3, hide: true },
    {
      ...
      filterable: false,
      renderCell: (index) =>
        index.api.getRowIndex(index.row.QCId) + 1 + (standardQCState.page - 1) * standardQCState.pageSize,
    },
    {
      field: 'action',
      headerName: '',
