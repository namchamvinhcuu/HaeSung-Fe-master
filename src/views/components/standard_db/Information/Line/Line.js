...
const Line = (props) => {
  ...
  useEffect(() => {
    if (!_.isEmpty(selectedRow) && !_.isEqual(selectedRow, LineDto)) {
      ...
      const index = _.findIndex(newArr, (o) => {
        return o.LineId == selectedRow.LineId;
      });
      if (index !== -1) {
  ...
  const columns = [
    ...
    {
      field: 'Description',
      headerName: intl.formatMessage({ id: 'line.Description' }),
      width: 500,
      renderCell: (params) => {
        return (
          <Tooltip title={params.row.Description ?? ''} className="col-text-elip">
            <Typography sx={{ fontSize: 14, maxWidth: 600 }}>{params.row.Description}</Typography>
          </Tooltip>
        );
      },
    },
