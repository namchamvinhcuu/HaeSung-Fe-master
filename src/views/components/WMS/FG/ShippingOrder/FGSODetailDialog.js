...
const FGSODetailDialog = (props) => {
  ...
  const { initModal, isOpen, onClose, setNewData, setUpdateData, mode, FGsoId } = props;
  ...
  const getMaterialTypeFinish = async () => {
  ...
  const onSubmit = async (data) => {
    ...
    const dataPost = { FGsoId: FGsoId, LotDtos: [...lotArr] };

    //setDialogState({ ...dialogState, isSubmit: true });

    const res = await fgSOService.createFGSODetail(dataPost);
    ...
    //     const dataPush = { ...data, FGsoId: FGsoId, LotIds: [...LotIds] };

    //     console.log(dataPush);

    //     const res = await fgSOService.createMsoDetail(dataPush);
    //     if (res.HttpResponseCode === 200 && res.Data) {
    //       SuccessAlert(intl.formatMessage({ id: res.ResponseMessage }));
    //       setNewData([...res.Data]);
    //       setDialogState({ ...dialogState, isSubmit: false });
    ...
    //   }
    //   else {
    //     const res = await fgSOService.modifyMsoDetail({
    //       ...data,
    //     });
  ...
  const getLots = async (materialId) => {
    setIsLoading(true);
    const res = await fgSOService.getLots(materialId);
    if (res && res.Data) {
      setLotArr([...res.Data]);
    } else setLotArr([]);
        ...
        <Grid container rowSpacing={2.5} columnSpacing={{ xs: 1, sm: 2, md: 3 }}>
          <Grid item xs={12}>
            <MuiAutocomplete
              ...
              fetchDataFunc={getMaterialTypeFinish}
...
export default FGSODetailDialog;
