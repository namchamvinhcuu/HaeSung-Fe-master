...
const WIPReportDetail = ({ LotId }) => {
  ...
  async function fetchData(LotId) {
    ...
    const res = await wipReportService.getReportDetail(params);
  ...
  const handleDownloadDetail = async () => {
    try {
      ...
      await wipReportService.downloadReportDetail(params);
    } catch (error) {
...
export default connect(mapStateToProps, mapDispatchToProps)(WIPReportDetail);
