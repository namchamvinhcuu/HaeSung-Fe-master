...
import { wipShipToNGService } from '@services';
...
const WIPShipToNG = (props) => {
  ...
  const fetchData = async () => {
    ...
    const res = await wipShipToNGService.get(params);
  ...
  const handleReceivingLot = async (inputValue) => {
    const res = await wipShipToNGService.receivingLot({ LotId: String(inputValue) });
    if (res && isRendered) {
  ...
  const getMaterial = async () => {
    const res = await wipShipToNGService.getMaterial();
    return res;
  };
