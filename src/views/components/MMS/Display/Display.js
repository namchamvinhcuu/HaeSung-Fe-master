  ...
  useEffect(() => {
    ...
    data?.forEach((ele) => {
      const index = rowData.findIndex(
        (x) =>
          x.woProcess == ele.woProcess && parseInt(ele?.lineName?.split('#')[1]) == parseInt(x?.lineName?.split('#')[1])
      );

      if (index == -1 && parseInt(ele?.lineName?.split('#')[1]) > 0) {
        let efficiency = Math.round((ele.actualQty / ele.orderQty) * 100);
        rowData.push({ ...ele, efficiency });
      }
    });
    let totalOrderQty = rowData.length > 0 ? rowData.map((x) => x.orderQty).reduce((a, b) => a + b) : 0;
    let totalActualQty = rowData.length > 0 ? rowData.map((x) => x.actualQty).reduce((a, b) => a + b) : 0;
    setDisplayData({
      data: rowData,
      totalOrderQty,
      totalActualQty,
      totalEfficiency: Math.round((totalActualQty / totalOrderQty) * 100),
    });
    console.log(rowData);
  }, [data]);
          ...
          <div style={{ height: '95%' }}>
            <div style={{ height: '13%', display: 'flex' }}>
              <div style={{ ...style.grid, width: '33.3333%', backgroundColor: '#9370db' }}>
                ...
                <h1 style={{ fontWeight: '600' }}>{Number(displayData.totalOrderQty).toLocaleString()}</h1>
              </div>
              <div style={{ ...style.grid, width: '33.3333%', backgroundColor: '#00c6bb' }}>
                <h2 style={{ fontWeight: '600' }}>Total Actual</h2>
                <h1 style={{ fontWeight: '600' }}>{Number(displayData.totalActualQty).toLocaleString()}</h1>
              </div>
              <div style={{ ...style.grid, width: '33.3333%', backgroundColor: '#e9a424' }}>
                <h2 style={{ fontWeight: '600' }}>Avg Efficiency</h2>
                <h1 style={{ fontWeight: '600' }}>
                  {displayData.totalEfficiency > 100 ? 100 : Math.round(displayData.totalEfficiency)}%
                </h1>
              </div>
            </div>
                  ...
                  displayData.data.map((item, index) => {
                    return (
                      <div style={{ width: '100%', height: '100%' }} key={index}>
                        ...
                        <div style={{ ...style.grid, height: '14.22%', display: 'grid', border: 'solid 1px #222222' }}>
                          <h2 style={{ fontWeight: '600' }}>{item.efficiency}%</h2>
                        </div>
                      </div>
                    );
                  })}
