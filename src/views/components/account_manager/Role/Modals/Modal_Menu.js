...
const Modal_Menu = ({ isShowing, hide, data, refreshTable }) => {
  ...
  return (
    <DraggableDialog animate="slide_down" isShowing={isShowing} hide={hide} onSave={handleSave} title={'Add Menu'}>
            ...
            return (
              <div key={'group' + params.key}>
                <div style={{ textIndent: '10px', marginBottom: 10 }}>
                  <span style={{ fontSize: 14 }} className="badge badge-primary">
                    {params.group}
                  </span>
                </div>

                <div style={{ textIndent: '30px', marginBottom: 10 }}>{params.children}</div>
              </div>
            );
          }}
          onChange={(item) => {
