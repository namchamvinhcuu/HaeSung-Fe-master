  ...
  const columns = [
    {
      ...
      renderCell: (params) => {
        return params.row.AdminType !== 1 ? (
          <Button variant="contained" color="primary" size="small" onClick={() => editrow(params)}>
            Edit
          </Button>
        ) : (
          ''
    ...
    },
    {
      ...
      renderCell: (params) => {
        return params.row.AdminType !== 1 ? (
          <Button variant="contained" color="error" size="small" onClick={() => deleterow(params)}>
            Delete
          </Button>
        ) : (
          ''
    ...
    },
    {
      ...
      renderCell: (params) => {
        return (
          <Button variant="contained" color="warning" size="small" onClick={() => changepassword(params)}>
            Change Password
          </Button>
        );
      },
...

    {
      ...
      renderCell: (params) => {
        return params.row.AdminType !== 1 ? (
          <Button variant="contained" color="primary" size="small" onClick={() => changeroles(params)}>
            Roles
          </Button>
        ) : (
          ''
    ...
    { field: 'FullName', headerName: 'Full name', width: 140, editable: false },
    {
      ...
      renderCell: (params) => {
        if (params.row.AdminType == 1)
          return (
            <span style={{ fontSize: 14 }} className="badge badge-danger">
              Super Admin
            </span>
          );
        else if (params.row.AdminType == 2)
          return (
            <span style={{ fontSize: 12 }} className="badge badge-secondary">
              Customer Admin
            </span>
          );
  ...
  return (
    <Box sx={{ pb: 5, height: '750px', width: '100%' }}>
      <Grid container direction="row" justifyContent="space-between" alignItems="center">
        <Grid item>
          <Button variant="contained" color="success" sx={{ boxShadow: 1, borderRadius: 2, mb: 1 }} onClick={addnew}>
            Add new
          </Button>
        </Grid>
        <Grid item>
          ...
          />
          <Button variant="contained" color="primary" sx={{ mx: 3, boxShadow: 1, mb: -4 }} onClick={search}>
            Search
          </Button>
        </Grid>
      </Grid>

      <DataTable ref={gridRef} url="account/getlist" columns={columns} />

      <Modal_UserAccount isShowing={isShowing} hide={toggle} mode={mode} data={rowdata} refreshTable={refreshTable} />
      <Modal_ChangePassword isShowing={isShowing2} hide={toggle2} data={rowdata} refreshTable={refreshTable} />

      <Modal_Roles isShowing={isShowing3} hide={toggle3} data={rowdata} />
    </Box>
  );
}
